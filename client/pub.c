...
int main(int argc, char* argv[])
{
	...
	printf("this is mqtt client for pub\n");
	mqtt_init(&broker, "avengalvon");
	//mqtt_init_auth(&broker, "cid", "campeador");
	//init_socket(&broker, "m2m.eclipse.org", 1883);
	init_socket(&broker, "198.41.30.241", 1883);
	...
	if(packet_buffer[3] != 0x00)//connect accept
	{
		fprintf(stderr, "CONNACK failed!\n");
		return -2;
	}
	printf("connnect ack\n");
	hexdump(packet_buffer,packet_length);
